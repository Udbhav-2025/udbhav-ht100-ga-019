...
5. Is compelling and action-oriented`;
  ...
  private async callLLM(prompt: string): Promise<string> {
    if (LLM_PROVIDER === 'gemini' && gemini) {
      ...
      throw new Error('No LLM provider configured. Set GEMINI_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY in .env');
    }
  }
  ...
  private async callGemini(prompt: string): Promise<string> {
    ...
    const fullPrompt = `You are an expert marketing AI that generates high-quality ad content. Always respond with valid JSON when requested.
